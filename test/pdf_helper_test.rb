...
class PdfHelperTest < ActionController::TestCase
  ...
  test "should prerender page object" do
    options = @ac.send( :prerender_page, :html => { :template => 'hf.html.erb'})
    assert !options[:html].has_key?(:template)
    assert_match /^file:\/\/.*hf.*\.html/, options[:html][:url]
  end
end
